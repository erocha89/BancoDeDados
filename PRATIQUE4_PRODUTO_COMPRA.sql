...
CREATE TABLE `PRODUTO_COMPRA` (
  `COD_PRODUTO_COMPRA` int NOT NULL AUTO_INCREMENT,
  `QTDA` int NOT NULL,
  `VLR_UNITARIO` decimal(10,2) DEFAULT NULL,
  `NUM_COMPRA` int NOT NULL,
  `COD_PRODUTO` int NOT NULL,
  PRIMARY KEY (`COD_PRODUTO_COMPRA`),
  KEY `FK_NUMCOMPRA` (`NUM_COMPRA`),
  KEY `FK_CODPRODUTO` (`COD_PRODUTO`),
  CONSTRAINT `FK_CODPRODUTO` FOREIGN KEY (`COD_PRODUTO`) REFERENCES `PRODUTO` (`COD_PRODUTO`),
  CONSTRAINT `FK_NUMCOMPRA` FOREIGN KEY (`NUM_COMPRA`) REFERENCES `COMPRA` (`NUM_COMPRA`)
